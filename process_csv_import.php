...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'import_csv') {
    ...
    // Expected CSV headers (added 'rfid')
    $expectedHeaders = ['student_no', 'fname', 'mname', 'lname', 'bday', 'year_level', 'strand_course', 'personal_email', 'rfid'];
    if (!$header || count(array_intersect($expectedHeaders, $header)) < 8) {
        fclose($csvFile);
    ...
    while (($row = fgetcsv($csvFile)) !== FALSE) {
        ...
        if (empty($student_no) || empty($fname) || empty($lname_display) || empty($bday) || empty($year_level) || empty($strand_course) || empty($personal_email) || empty($rfid)) {
            $results['errors']++;
        ...
        // Check duplicates: student_no OR rfid
        $check = $con->prepare("SELECT student_no FROM form WHERE student_no = ? OR rfid = ?");
        $check->bind_param("ss", $student_no, $rfid);
        $check->execute();
        $exists = $check->get_result();

        if ($exists->num_rows > 0) {
            $results['duplicates']++;
            $results['error_details'][] = "Row $rowNumber: Duplicate student_no or RFID";
            continue;
        }

        // Generate portal email and password
        ...
        // Insert into DB
        $stmt = $con->prepare("INSERT INTO form (student_no, fname, mname, lname, bday, email, pass, year_level, strand_course, personal_email, rfid) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param(
            "sssssssssss",
            $student_no, $fname, $mname, $lname_display, $bday,
            $email, $hashed_password, $year_level, $strand_course, $personal_email, $rfid
        );
    ...
    if ($results['duplicates'] > 0) $message .= "• Duplicates skipped: {$results['duplicates']}<br>";
    if ($results['email_errors'] > 0) $message .= "• Email sending errors: {$results['email_errors']}<br>";
    if ($results['errors'] > 0) $message .= "• Processing errors: {$results['errors']}<br>";

    if (!empty($results['error_details'])) {
        $message .= "<br><strong>Error Details:</strong><br>";
        $displayErrors = array_slice($results['error_details'], 0, 10);
        foreach ($displayErrors as $error) $message .= "• $error<br>";
