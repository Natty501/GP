<?php
session_start();
include 'db.php'; // make sure you have your database connection here
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if(isset($_SESSION['email'])) {
        ...
        // Example category scoring (adjust indices as needed)
        $score_stress = array_sum(array_slice($answers, 0, 4));  // Q1-4
        $score_mood   = array_sum(array_slice($answers, 4, 4));  // Q5-8
        $score_sleep  = array_sum(array_slice($answers, 8, 2));  // Q9-10

        $stmt = $con->prepare("INSERT INTO mh_history (user_email, score_total, score_stress, score_mood, score_sleep) VALUES (?, ?, ?, ?, ?)");
        $stmt->bind_param("siiii", $email, $score, $score_stress, $score_mood, $score_sleep);
        $stmt->execute();
    }
