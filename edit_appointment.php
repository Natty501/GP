          ...
          <form action="update_appointment.php" method="POST" class="space-y-8">
            ...
            <div class="bg-green-50 rounded-xl p-6">
              ...
              <div class="space-y-6">
                ...
                <div>
  <label class="block text-sm font-medium text-gray-700 mb-2">Guidance Service</label>
  <select name="guidance_services" id="guidanceService" class="form-input w-full px-4 py-3 rounded-xl bg-white" required>
    ...
    <option value="Referral Service">Referral Service</option>
                ...
                <?php $reasonDisplay = !empty($data['reason']) ? 'block' : 'none'; ?>
    <div id="reasonDiv" style="display: <?= $reasonDisplay ?>;">
      ...
      <textarea name="reason" class="form-input w-full px-4 py-3 rounded-xl bg-white resize-none" 
        rows="4" placeholder="Please describe what you'd like to discuss..."><?= htmlspecialchars($data['reason']) ?></textarea>
    </div>
  </div>
</div>
...
<script>
  ...
  function toggleSubCategory() {
    ...
    // Preselect previous value if exists
    const currentSpecific = "<?= addslashes($data['specific_concern']) ?>";
    if (currentSpecific) {
      sub.value = currentSpecific;
    }
  }
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Call toggleSubCategory to load previous selections
    toggleSubCategory();

    // Preselect Guidance Service
    const guidanceSelect = document.getElementById("guidanceService");
    const currentService = "<?= addslashes($data['guidance_services']) ?>";
    if (guidanceSelect && currentService) {
      guidanceSelect.value = currentService;
    }

    // Hide Reason for Consultation if empty
