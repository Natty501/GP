...
<!-- Floating Background Elements -->
  ...
  <header class="glass fixed top-0 left-0 w-full z-50 border-b border-white/20">
    <div class="container mx-auto flex justify-between items-center py-4 px-6">
      ...
      <nav class="hidden md:flex space-x-6 animate-fadeInUp">
        <a href="home.php" class="text-white/90 hover:text-white transition-colors font-medium relative group px-4 py-2 rounded-lg hover:bg-white/10">
          Home
  ...
  <div id="errorModal" class="fixed inset-0 z-50 flex items-center justify-center hidden bg-black/50 backdrop-blur-sm">
  <div class="bg-white rounded-2xl p-8 max-w-sm w-full mx-4 shadow-2xl transform transition-all scale-95 opacity-0" id="errorModalContent">
    <div class="text-center">
      ...
      <p id="errorModalMessage" class="text-gray-600 mb-6">Account not found. Please contact Guidance Office.</p>
      <button id="closeErrorModal" class="w-full py-3 px-6 bg-gray-900 text-white rounded-xl font-semibold hover:bg-gray-800 transition-colors">
        Got it
      </button>
    </div>
  </div>
...
<script>
    ...
    // Function to hide the custom error modal
    ...
    // Step 1: Username submission with AJAX Check
    usernameForm.addEventListener('submit', (e) => {
      ...
      if (identifier) {
        ...
        // AJAX request to check if user is enrolled
        ...
        .then(data => {
            if (data.status === 'success') {
                // ✅ User found! Proceed to password modal
                ...
                setTimeout(() => {
                    ...
                    // Display FIRST NAME from database
                    displayUsername.textContent = data.fname; 
                    passwordInput.focus();
                ...
                // ❌ Modal Alert instead of standard alert
        ...
        .catch(error => {
            ...
            showError("A connection error occurred. Please check your internet.");
            continueButton.disabled = false;
            continueButton.innerHTML = 'Continue';
        });
      }
