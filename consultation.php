    ...
    <table id="consultationTable">
      <thead>
        <tr>
          ...
          <th><i class="bi bi-list-check"></i> Guidance Service</th>
      ...
      <tbody>
        ...
        <td>" . htmlspecialchars($row['guidance_services']) . "</td>
        ...
        <td id='status-{$row['id']}'>" . htmlspecialchars($row['status'] ?: 'Pending') . "</td>
        <td><button class='btn-manage' onclick=\"openModal('{$row['id']}')\"><i class='bi bi-gear'></i> Manage</button></td>
      </tr>";

          }
        } else {
          echo "<tr><td colspan='12' class='no-appointments'>
...
function submitForm() {
  ...
  const adminMessage = document.querySelector('textarea[name="admin_message"]').value;
  ...
  .then(data => {
    if (data.success) {
      ...
      // Optionally disable Manage button after decision
      const manageBtn = statusCell.nextElementSibling.querySelector('.btn-manage');
