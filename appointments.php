...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $guidance_services = $_POST['guidance_services'];
    ...
    if ($time < "08:00" || $time > "17:00") {
        ...
        $stmt = $con->prepare("INSERT INTO appointments (name, email, student_no, date, time, grade, section, interest, reason, teacher, guidance_services) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssssssssss", $name, $email, $student_no, $date, $time, $grade, $section, $interest, $reason, $teacher, $guidance_services);
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div class="appointment-card p-8 animate-fadeInUp">
          <div class="text-center">
            ...
            <h3 class="text-xl font-bold text-gray-800 mb-2">Appointment</h3>
        ...
        <form action="add_appointment.php" method="POST" class="space-y-6">
          ...
          <div class="bg-purple-50 rounded-xl p-6">
            ...
            <div class="space-y-4">
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-3">Guidance Service</label>
                <select name="guidance_services" class="form-input w-full px-4 py-3 rounded-xl bg-white" required>
                  <option value="">-- Select Guidance Service --</option>
                  <option value="Information Service">Information Service</option>
                  <option value="Individual Counseling/Consultation">Individual Counseling/Consultation</option>
                  <option value="Group Counseling/Consultation">Group Counseling/Consultation</option>
              ...
              <div id="teacherDiv" style="display:none;">
                ...
                <input type="text" name="teacher" class="form-input w-full px-4 py-3 rounded-xl bg-white" placeholder="Enter teacher's name (optional)">
              ...
              <div id="otherReasonDiv" style="display:none;">
                ...
                <textarea name="reason" class="form-input w-full px-4 py-3 rounded-xl bg-white resize-none" placeholder="Please describe what you'd like to discuss or any specific concerns you have..." rows="4"></textarea>
              </div>
            </div>
          </div>
...
<div id="weekendModal"
  ...
  <div class="bg-white p-6 rounded-xl max-w-sm w-full text-center shadow-2xl">
    <p class="text-gray-700 mb-6">
      Appointments cannot be scheduled on Saturdays or Sundays.
    </p>

    <button id="closeModal"
            class="px-8 py-2 bg-blue-500 text-white font-semibold rounded-full
...
<!-- Lunch Break Modal -->
<div id="lunchModal" class="fixed inset-0 hidden bg-black/50 flex items-center justify-center z-50">
  <div class="bg-white p-6 rounded-xl max-w-sm w-full text-center shadow-2xl">
    <p class="text-gray-700 mb-6">
      Appointments cannot be scheduled during lunch break (12:00 PM – 1:00 PM). Please choose another time.
    </p>
    <button id="closeLunchModal"
            class="px-8 py-2 bg-blue-500 text-white font-semibold rounded-full
                   hover:bg-blue-600 transition duration-200 shadow-md">
      OK
    </button>
...
<script>
 ...
 function toggleSubCategory() {
    ...
    const teacherDiv = document.getElementById("teacherDiv"); // Teacher div
    const reasonTextarea = document.querySelector('textarea[name="reason"]');
    const specificSelect = document.querySelector('select[name="specific_concern"]');
    ...
    if (main === "Personal") {
        ...
        teacherDiv.style.display = "none";
        reasonTextarea.required = true;  // Make textarea required
        return;  // Exit here since no subcategory options
    }

    // Populate subcategory if options exist
    if (options.length > 0) {
        ...
        specificSelect.required = true; // Make subcategory select required
    } else {
        subDiv.style.display = "none";
        otherDiv.style.display = "none";
    }
...
const closeLunchBtn = document.getElementById('closeLunchModal');
...
if(timeInput){
  timeInput.addEventListener('input', function(){
    ...
    if(selectedTime >= "12:00" && selectedTime < "13:00"){
      ...
      lunchModal.style.display = 'flex'; // Show modal
    }
  });
}

if(closeLunchBtn){
  closeLunchBtn.addEventListener('click', function(){
