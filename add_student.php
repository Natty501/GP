...
// Initialize modal message variable
$modal_msg = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($exists->num_rows > 0) {
        $_SESSION['modal_msg'] = "Student number already registered.";
        ...
        $stmt = $con->prepare("INSERT INTO form 
            (student_no, fname, mname, lname, bday, email, pass, year_level, strand_course, personal_email, rfid) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param(
            "sssssssssss",
            $student_no, $fname, $mname, $lname_display, $bday,
            $email, $hashed_password, $year_level, $strand_course, $personal_email, $_POST['rfid']
        );
        ...
        if ($stmt->execute()) {
            ...
            try {
                ...
                $mail->Password = 'dcxzvdssbkfembun';
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;
                ...
                $_SESSION['modal_msg'] = "Student added successfully!<br>Email: $email<br>Password: $raw_password";
            } catch (Exception $e) {
                $_SESSION['modal_msg'] = "Student added but email could not be sent.<br>Error: {$mail->ErrorInfo}";
            }
        } else {
            $_SESSION['modal_msg'] = "Error: Failed to insert student.";
        }
...
// Get modal message from session if available
if(isset($_SESSION['modal_msg'])) {
    $modal_msg = $_SESSION['modal_msg'];
    unset($_SESSION['modal_msg']); // clear after showing
}
                    ...
                    <div class="col-lg-4">
                        <div class="csv-template">
                            ...
                            <div class="text-white small">
                                ...
                                <code>personal_email</code><br>
                                <code>rfid</code> (Student RFID Number)
                            </div>
      ...
      <form method="POST" action="add_student.php">
        ...
        <div class="modal-header bg-primary text-white">
          <h5 class="modal-title">
            <i class="bi bi-person-plus"></i> Add Student (Manual)
          </h5>
...
<script>
    ...
    // File input change
    csvFile.addEventListener('change', (e) => { if(e.target.files.length) handleFile(e.target.files[0]); });

    function handleFile(file) {
        if(!file.name.toLowerCase().endsWith('.csv')) return showError('Please select a valid CSV file.');
    ...
    function processCSV(file) {
        ...
        fetch('process_csv_import.php', { method: 'POST', body: formData })
        .then(res => res.json())
        .then(data => {
            ...
            if(data.success) {
                ...
                successModal.addEventListener('hidden.bs.modal', () => {
                    // Redirect after modal close to avoid "continue form submission" popup
                    window.location.href = window.location.href.split('?')[0];
                });
            } else showError(data.message);
        })
        .catch(err => { progressContainer.style.display = 'none'; showError('An error occurred while processing the file.'); console.error(err); });
        ...
        setTimeout(() => { clearInterval(interval); progressBar.style.width = '100%'; progressText.textContent = 'Finalizing...'; }, 10000);
    }
    ...
    function downloadTemplate() {
        const csvContent = "student_no,fname,mname,lname,bday,year_level,strand_course,personal_email,rfid\n" +
                           "2024001234,John,Michael,Doe,2005-01-15,Grade 11,STEM,john.doe@email.com,1234567890\n" +
                           "2024001235,Jane,,Smith,2004-12-20,1st year,BSIT,jane.smith@email.com,0987654321";
    ...
    // Show modal after PHP redirect (POST-Redirect-GET)
    <?php if (!empty($modal_msg)): ?>
